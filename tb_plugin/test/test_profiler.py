...
class TestProfiler(unittest.TestCase):
    ...
    def test_gpu_utilization(self):
        json_content = """
          ...
          {
            "ph": "X", "cat": "Kernel",
            "name": "void cunn_ClassNLLCriterion_updateGradInput_kernel<float>", "pid": 0, "tid": "stream 8",
            "ts": 135, "dur": 15,
        ...
        sm_efficiency_id = 0
        with gzip.open(profile.trace_file_path) as file:
            trace_events = json.load(file)
            trace_events = trace_events["traceEvents"]
            for event in trace_events:
                if event["ph"] == "C":
                    if "Utilization" in event["name"]:
                        self.assertTrue(gpu_util_id < len(gpu_util_expected))
                        self.assertAlmostEqual(event["ts"], gpu_util_expected[gpu_util_id][0])
                        self.assertAlmostEqual(event["args"]["GPU Utilization"], gpu_util_expected[gpu_util_id][1])
                        gpu_util_id += 1
                    if "Est. SM Efficiency" in event["name"]:
                        self.assertTrue(sm_efficiency_id < len(sm_efficiency_expected))
                        self.assertAlmostEqual(event["ts"], sm_efficiency_expected[sm_efficiency_id][0])
                        self.assertAlmostEqual(event["args"]["Est. SM Efficiency"],
                                               sm_efficiency_expected[sm_efficiency_id][1])
                        sm_efficiency_id += 1
